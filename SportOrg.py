...
def _qt_widgets():
    """Load Qt widgets from either supported binding."""
    try:
        from PySide6.QtWidgets import QApplication, QMessageBox
    except ModuleNotFoundError:
        try:
            from PySide2.QtWidgets import QApplication, QMessageBox  # type: ignore[import-not-found]
        except ModuleNotFoundError as error:
            raise RuntimeError(
                "Neither PySide6 nor PySide2 is installed. Install the GUI extras."
            ) from error
...
def report_fatal(error) -> None:
    ...
    try:
        QApplication, QMessageBox = _qt_widgets()
    except RuntimeError:
        return
