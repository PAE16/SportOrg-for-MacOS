...
+ Добавлено приложение `SportOrg for MacOS` и сборка установочного DMG-образа.
+ Пакет для MacOS является универсальным и поддерживает Apple Silicon (`arm64`) и Intel (`x86_64`).
+ Добавлена генерация и упаковка иконки приложения MacOS через `cx_Freeze`.
+ Добавлены инструкции по установке и локальной сборке DMG.
+ Добавлены встроенные шаблоны протоколов и корректный поиск шаблонов, звуков, конфигурации и логов относительно установленного приложения.
+ Добавлен нормализованный слой преобразования SportOrg в SFR для данных соревнования, групп, команд и участников.
+ Экспорт SFR стал безопаснее при отсутствии команды, группы, разряда, даты рождения или времени.
