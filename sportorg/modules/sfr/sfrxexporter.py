...
def _write_days(f, race: Race):
    ...
    day_fields = ["p01", discipline, date_str, w_url]
...
def _write_courses(f, race: Race):
    for i, course in enumerate(race.courses):
        ...
        course_fields = [
            f"d{course_id_str}",
            course_name,
            str(getattr(course, "bib", "0") or "0"),
            "1",
            "0",
            "0",
            str(getattr(course, "length", "0") or "0"),
            str(getattr(course, "climb", "0") or "0"),
...
def _get_time_part(
    obj: object, name: str, default: Optional[int] = 0
) -> Optional[int]:
...
def _convert_to_datetime(time_obj):
    ...
    try:
        ...
        if callable(to_time):
            ...
            if isinstance(converted_time, time):
                return datetime.combine(datetime.today().date(), converted_time)
...
class SportOrgToSFR:
    @staticmethod
    def RaceToSFR(race, log=None):
        """Convert SportOrg race data into a normalized SFR-like dictionary.

        The structure mirrors the original C# adapter contract: it groups meta,
        groups, teams and persons in one payload while keeping a lightweight log
        stream for warnings and skipped records.
        """
        if race is None:
            return {"meta": {}, "groups": [], "teams": [], "persons": []}

        log_buffer = [] if log is None else log

        meta = {
            "title": getattr(getattr(race, "data", None), "title", "") or "",
            "location": getattr(getattr(race, "data", None), "location", "") or "",
            "start_date": getattr(getattr(race, "data", None), "start_datetime", None),
            "race_type": getattr(getattr(race, "data", None), "race_type", None),
            "description": getattr(getattr(race, "data", None), "description", "") or "",
        }

        if hasattr(meta["start_date"], "strftime"):
            meta["start_date"] = meta["start_date"].strftime("%Y-%m-%d")

        groups_by_name = {}
        groups = []
        for index, group in enumerate(getattr(race, "groups", []) or []):
            mapped_group = SportOrgToSFR.GroupToSFR(index, group)
            groups.append(mapped_group)
            groups_by_name.setdefault(group.name if getattr(group, "name", None) else f"group_{index}", index)

        teams_by_name = {}
        teams = []
        for index, org in enumerate(getattr(race, "organizations", []) or []):
            mapped_team = SportOrgToSFR.TeamToSFR(index, org)
            teams.append(mapped_team)
            teams_by_name.setdefault(org.name if getattr(org, "name", None) else f"team_{index}", index)

        persons = []
        for index, person in enumerate(getattr(race, "persons", []) or []):
            group_id = 0
            team_id = 0

            if getattr(person, "group", None) is not None:
                group_name = getattr(person.group, "name", None) or f"group_{index}"
                group_id = groups_by_name.get(group_name, 0)

            if getattr(person, "organization", None) is not None:
                team_name = getattr(person.organization, "name", None) or f"team_{index}"
                team_id = teams_by_name.get(team_name, 0)

            mapped_person = SportOrgToSFR.PersonToSFR(
                index,
                group_id,
                team_id,
                person,
                getattr(race, "groups", []) or [],
                getattr(race, "organizations", []) or [],
            )
            persons.append(mapped_person)

            if getattr(person, "organization", None) is None:
                log_buffer.append(f"Person {getattr(person, 'bib', index)} has no team assigned")
        ...
        if log is not None and isinstance(log, list):
            log[:] = log_buffer
        elif log is not None and isinstance(log, str):
            log = "\n".join(log_buffer)

        return result

    @staticmethod
    def PersonToSFR(id_value, group_id_sfr, organization_id_sfr, person, groups, organizations):
        person_name = getattr(person, "name", "") or ""
        person_middle_name = getattr(person, "middle_name", "") or ""
        person_surname = getattr(person, "surname", "") or ""
        birthday = getattr(person, "birth_date", None)
        if birthday is not None and hasattr(birthday, "strftime"):
            birthday = birthday.strftime("%Y-%m-%d")

        start_time = getattr(person, "start_time", None)
        if start_time is not None and hasattr(start_time, "strftime"):
            start_time = start_time.strftime("%H:%M:%S")

        result = {
            "id": id_value,
            "bib": getattr(person, "bib", 0) or 0,
            "group_id": group_id_sfr,
            "team_id": organization_id_sfr,
            "surname": person_surname,
            "name": person_name,
            "middle_name": person_middle_name,
            "birthday": birthday or "",
            "qual_id": sportorg_qual_to_sfr(getattr(person, "qual", None)),
            "comment": getattr(person, "comment", "") or "",
            "start_time": start_time or "",
            "group": getattr(person, "group", None),
            "team": getattr(person, "organization", None),
            "groups": groups,
    ...
    @staticmethod
    def GroupToSFR(id_value, group):
        return {
            "id": id_value,
            "name": getattr(group, "name", None) or f"Group_{id_value}",
            "course": getattr(group, "course", None),
        }

    @staticmethod
    def TeamToSFR(id_value, team):
        return {
            "id": id_value,
            "name": getattr(team, "name", None) or f"Team_{id_value}",
        }


def sportorg_qual_to_sfr(qual: Optional[Qualification]) -> str:
    if qual is None:
        return "0"
