...
class TestSFRxExporter:
    ...
    def test_export_sfrx_success(self, sfrxexporter):
        ...
        team = next(row for row in rows if row[0].startswith("t"))
        assert team[3:7] == ["", "", "", ""]
        assert len(rows[0]) == 10
        assert len(next(row for row in rows if row[0].startswith("g"))) == 14
        assert len(team) == 9
        assert len(competitor) == 21
        assert not any(row[0].startswith(("d", "k", "s")) for row in rows)
    ...
    def test_export_sfrx_uses_zero_status_for_non_ok_results(self, sfrxexporter):
        self.result1.status = ResultStatus.DISQUALIFIED
        self.result1.finish_time = None
        self.result1.get_finish_time.return_value = None
        sfrxexporter.memory.race.return_value = self.race
        sfrxexporter.translate = lambda x: x

        sfrxexporter.export_sfrx(self.test_file)

        rows = self.read_rows()
        competitor = next(row for row in rows if row[0].startswith("c"))

        assert competitor[14] == "0"
        assert competitor[16] == "0"
    ...
    def test_sportorg_to_sfr_maps_groups_teams_and_persons(self, sfrxexporter):
        mapped = sfrxexporter.SportOrgToSFR.RaceToSFR(self.race, None)

        assert mapped["meta"]["title"] == "Тестовые соревнования"
        assert mapped["groups"][0]["name"] == "М21"
        assert mapped["teams"][0]["name"] == "СК Приз"
        assert mapped["persons"][0]["bib"] == 101
        assert mapped["persons"][0]["group_id"] == 0
        assert mapped["persons"][0]["team_id"] == 0
